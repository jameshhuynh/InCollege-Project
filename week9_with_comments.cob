>>SOURCE FORMAT FREE
       ...
       FILE-CONTROL.
           ...
           SELECT TEMP-PROFILE-FILE ASSIGN TO "temp_profiles.dat"
                 ORGANIZATION IS LINE SEQUENTIAL
                 FILE STATUS IS WS-TEMP-PROF-STATUS.
       ...
       FD  CONNECTION-FILE.
       01  CONN-REC       PIC X(60).

       FD TEMP-CONNECTION-FILE.
       01 TEMP-CONN-REC      PIC X(60).

       FD TEMP-PROFILE-FILE.
       01 TEMP-PROFILE-REC    PIC X(900).
       ...
       *> ============================================================
       01  WS-JOB-POSTING.
          ...
          05 JP-DESCRIPTION    PIC X(200).
          05 JP-EMPLOYER       PIC X(50).
       ...
       01  WS-JOB-TABLE.
          05 WS-JOB-ENTRY OCCURS 99 TIMES.
             10 JT-POSTER         PIC X(20).
             10 JT-TITLE          PIC X(50).
             10 JT-DESCRIPTION    PIC X(200).
             10 JT-EMPLOYER       PIC X(50).
             10 JT-LOCATION       PIC X(50).
             10 JT-SALARY         PIC X(30).
        ...
        PARSE-PROFILE-REC.
            UNSTRING PROFILE-REC DELIMITED BY ","
                INTO WS-REC-USERNAME
                     ...
                     PF-EDU-YEARS(3)
            END-UNSTRING.
            MOVE FUNCTION TRIM(PF-UNIVERSITY) TO PF-UNIVERSITY
            MOVE FUNCTION TRIM(PF-MAJOR)      TO PF-MAJOR
            MOVE FUNCTION TRIM(PF-ABOUT-ME)   TO PF-ABOUT-ME

            MOVE FUNCTION TRIM(PF-EXP-TITLE(1)) TO PF-EXP-TITLE(1)
            MOVE FUNCTION TRIM(PF-EXP-TITLE(2)) TO PF-EXP-TITLE(2)
            MOVE FUNCTION TRIM(PF-EXP-TITLE(3)) TO PF-EXP-TITLE(3)

            MOVE FUNCTION TRIM(PF-EXP-DESC(1))  TO PF-EXP-DESC(1)
            MOVE FUNCTION TRIM(PF-EXP-DESC(2))  TO PF-EXP-DESC(2)
            MOVE FUNCTION TRIM(PF-EXP-DESC(3))  TO PF-EXP-DESC(3)

            MOVE FUNCTION TRIM(PF-EDU-DEGREE(1)) TO PF-EDU-DEGREE(1)
            MOVE FUNCTION TRIM(PF-EDU-DEGREE(2)) TO PF-EDU-DEGREE(2)
            MOVE FUNCTION TRIM(PF-EDU-DEGREE(3)) TO PF-EDU-DEGREE(3)

            MOVE FUNCTION TRIM(PF-EDU-UNIV(1))   TO PF-EDU-UNIV(1)
            MOVE FUNCTION TRIM(PF-EDU-UNIV(2))   TO PF-EDU-UNIV(2)
            MOVE FUNCTION TRIM(PF-EDU-UNIV(3))   TO PF-EDU-UNIV(3)

            MOVE FUNCTION TRIM(PF-EDU-YEARS(1))  TO PF-EDU-YEARS(1)
            MOVE FUNCTION TRIM(PF-EDU-YEARS(2))  TO PF-EDU-YEARS(2)
            MOVE FUNCTION TRIM(PF-EDU-YEARS(3))  TO PF-EDU-YEARS(3)
               ...
               *>--- Experience ---
               IF SF-EXP-COUNT > 0
                   ...
                   PERFORM VARYING WS-I FROM 1 BY 1 UNTIL WS-I >
                       ...
                       MOVE SPACES TO WS-DISPLAY-MESSAGE
                       STRING "Title: " DELIMITED BY SIZE
                              FUNCTION TRIM(SF-EXP-TITLE(WS-I))
                              DELIMITED BY SIZE
                              INTO WS-DISPLAY-MESSAGE
                       END-STRING
                       PERFORM WRITE-OUTPUT-AND-DISPLAY
                       ...
                       MOVE SPACES TO WS-DISPLAY-MESSAGE
                       STRING "Company: " DELIMITED BY SIZE
                              FUNCTION TRIM(SF-EXP-COMPANY(WS-I))
                              DELIMITED BY SIZE
                              INTO WS-DISPLAY-MESSAGE
                       END-STRING
                       PERFORM WRITE-OUTPUT-AND-DISPLAY
                       ...
                       MOVE SPACES TO WS-DISPLAY-MESSAGE
                       STRING "Dates: " DELIMITED BY SIZE
                              FUNCTION TRIM(SF-EXP-DATES(WS-I))
                              DELIMITED BY SIZE
                              INTO WS-DISPLAY-MESSAGE
                       END-STRING
                       PERFORM WRITE-OUTPUT-AND-DISPLAY
                       ...
                       IF SF-EXP-DESC(WS-I) NOT = SPACES
                           MOVE SPACES TO WS-DISPLAY-MESSAGE
                           STRING "Description: " DELIMITED BY SIZE
                                  FUNCTION TRIM(SF-EXP-DESC(WS-I))
                                  DELIMITED BY SIZE
                                  INTO WS-DISPLAY-MESSAGE
                           END-STRING
                           PERFORM WRITE-OUTPUT-AND-DISPLAY
               ...
               *>--- Education ---
               IF SF-EDU-COUNT > 0
                   ...
                   PERFORM VARYING WS-I FROM 1 BY 1 UNTIL WS-I >
                       ...
                       MOVE SPACES TO WS-DISPLAY-MESSAGE
                       STRING "Degree: " DELIMITED BY SIZE
                              FUNCTION TRIM(SF-EDU-DEGREE(WS-I))
                              DELIMITED BY SIZE
                              INTO WS-DISPLAY-MESSAGE
                       END-STRING
                       PERFORM WRITE-OUTPUT-AND-DISPLAY
                       ...
                       MOVE SPACES TO WS-DISPLAY-MESSAGE
                       STRING "University: " DELIMITED BY SIZE
                              FUNCTION TRIM(SF-EDU-UNIV(WS-I))
                              DELIMITED BY SIZE
                              INTO WS-DISPLAY-MESSAGE
                       END-STRING
                       PERFORM WRITE-OUTPUT-AND-DISPLAY
                       ...
                       MOVE SPACES TO WS-DISPLAY-MESSAGE
                       STRING "Years: " DELIMITED BY SIZE
                              FUNCTION TRIM(SF-EDU-YEARS(WS-I))
                              DELIMITED BY SIZE
                              INTO WS-DISPLAY-MESSAGE
                       END-STRING
                       PERFORM WRITE-OUTPUT-AND-DISPLAY
       ...
       *> Saves a new PENDING request from PF-USERNAME â†’ WS-FOUND-USERNAME
       *>****************************************************************
       SAVE-CONNECTION-REQUEST.
       ...
       *>****************************************************************
       ...
       VIEW-PENDING-CONNECTIONS.
            MOVE SPACES TO WS-DISPLAY-MESSAGE
            MOVE "--- Pending Connection Requests ---" TO
            WS-DISPLAY-MESSAGE
            PERFORM WRITE-OUTPUT-AND-DISPLAY
            ...
            PERFORM UNTIL EOF-CONNECTION-FILE = 'Y'
                READ CONNECTION-FILE INTO CONN-REC
                    ...
                    NOT AT END
                        ...
                        IF (FUNCTION TRIM(WS-REC-RECIPIENT) =
                            FUNCTION TRIM(PF-USERNAME))
                           AND (FUNCTION TRIM(WS-CONN-STATUS-FLAG) =
                            ...
                            MOVE SPACES TO WS-DISPLAY-MESSAGE
                            MOVE "2. Reject" TO WS-DISPLAY-MESSAGE
                            PERFORM WRITE-OUTPUT-AND-DISPLAY

                            DISPLAY "Enter your choice for "
                                    FUNCTION TRIM(WS-REC-SENDER) ": "
                                    WITH NO ADVANCING
                            ...
                            EVALUATE WS-USER-CHOICE
                                ...
                                WHEN '2'
                                    ...
                                    MOVE 'Y' TO EOF-CONNECTION-FILE
                                WHEN OTHER
                                    DISPLAY "Invalid choice, skipping."
                            END-EVALUATE
                        END-IF
                END-READ
            END-PERFORM

            IF EOF-CONNECTION-FILE = 'N'
                CLOSE CONNECTION-FILE
            END-IF
       ...
       *>****************************************************************
       ACCEPT-CONNECTION.
           ...
           *> Confirmation message
           MOVE SPACES TO WS-DISPLAY-MESSAGE
           STRING "âœ… Connection request from "
       ...
       *>****************************************************************
       REJECT-CONNECTION.
           ...
           MOVE SPACES TO WS-DISPLAY-MESSAGE
           STRING "âŒ Connection request from "
                  FUNCTION TRIM(WS-REQUEST-SENDER)
                  " has been REJECTED."
                  DELIMITED BY SIZE
                INTO WS-DISPLAY-MESSAGE
